		...
		try {
			...
			if (start_date != null && start_date != '') {
				if (end_date != null && end_date != '') {
					...
					start_date = res_start[2] + '-' + res_start[1] + '-'
							+ res_start[0];
					...
					var start_date_db = res_start[0] + '/' + res_start[1] + '/'
							+ res_start[2];
					var end_date_db = res_end[0] + '/' + res_end[1] + '/'
							+ res_end[2];
					if (end_date < start_date) {
						...
						var firstDate = new Date(res_start[2], res_start[1],
								res_start[0]);
						var secondDate = new Date(res_end[2], res_end[1],
								res_end[0]);
							...
							jQuery
									.ajax({
										...
										success : function(obj) {
											if (obj.length > 0) {
												document
														.getElementById('statusInformation').style.display = "block";
												...
												for (var i = 0; i < obj.length; i++) {
													...
													if (obj[i].status != '1'
															&& obj[i].status_approve != '10') {
														doc_no = '99' + doc_no;
													}
													obj[i].objective = obj[i].objective
															.replace(/"/g, '');
													console
															.log(obj[i].trans_id
																	+ '|'
																	...
																	+ obj[i].doc_no
																	+ '|'
																	+ obj[i].status_approve
																	+ '|'
																	+ obj[i].status
																	+ '|'
																	+ obj[i].approver
																	+ '|'
																	+ obj[i].approve_status_desc);
													str = str
															+ '<label  class="custom-font-label-thin-small">'
															+ obj[i].codempid
															+ ' '
															+ obj[i].namempt
															+ ' </label></div>';
													str = str
															+ '<div class = "col-md-1 col-sm-3 col-xs-12"><label class="custom-font-label '
															+ style_doc
															+ ' text-center" style="cursor:pointer;"  onclick="EditWorkOut(&quot;'
															+ obj[i].trans_id
															+ '|'
															+ obj[i].codempid
															+ '|'
															...
															+ '</label></div>';
													str = str
															+ '<div class = "col-md-11 col-sm-9 col-xs-12 custom-gap-width" style="padding-top:4px;"><label class="font-thin">'
															+ obj[i].start_date
															+ ' '
															+ obj[i].start_time
															+ ' - '
															...
															+ '</label></div>';
													str = str
															+ '<div class="col-md-12 col-sm-12 col-xs-12"><i class="glyphicon glyphicon-map-marker"></i>&nbsp;<label class="font-thin" style= padding-top:4px;padding-left:2px;border-radius: 5px;>'
															+ obj[i].place
															+ '</label></div>';
													str = str + '</li>';
												}
		...
		try {
			...
			if (count_datetime == 4) {
				...
				if (n_e_date < n_s_date) {
					...
					if (check == 0) {
						if ($("#txtDlgObjective").val() != null
								&& $("#txtDlgObjective").val() != '') {
							if ($("#txtDlgPlace").val() != null
									&& $("#txtDlgPlace").val() != '') {
								if ($("#txtDlgStartPlace").val() != null
										&& $("#txtDlgStartPlace").val() != '') {

									if ($("#txtDlgEndPlace").val() != null
											&& $("#txtDlgEndPlace").val() != '') {
										...
										var data = {}
										data["trans_id"] = $("#lblDlgTransID")
												.html();
										data["doc_no"] = $('#lblDlgDocNo')
												.html();
										data["codempid"] = $("#lblDlgEmpID")
												.html();
										data["company_id"] = $(
												"#lblDlgCompanyID").html();
										data["objective"] = $(
										...
										data["start_place"] = $(
												"#txtDlgStartPlace").val();
										data["end_place"] = $("#txtDlgEndPlace")
												.val();
										data["start_date"] = start_date_db;
										data["end_date"] = end_date_db;
										data["start_time"] = res_start_time[0]
												+ ':' + res_start_time[1];
										data["end_time"] = res_end_time[0]
												+ ':' + res_end_time[1];
										data["day_qty"] = parseInt($(
												'#lblDlgDayNum').html());
										data["save_case"] = x;
										data["status"] = $('#lblDlgStatus')
												.html();
										if (document.getElementById('r1').checked) {
											data["place_flag"] = document
													.getElementById('r1').value;
										} else if (document
												.getElementById('r2').checked) {
											data["place_flag"] = document
													.getElementById('r2').value;
										} else {
											data["place_flag"] = document
													.getElementById('r3').value;
										}

										if (document
												.getElementById('chkPrivate').checked) {
											data["private_flag"] = "Y";
										} else {
											data["private_flag"] = "N";
										}
